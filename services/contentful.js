...
jest.mock('../services/contentful', () => ({
  getClient: jest.fn(() => ({
    getEntries: jest.fn(() =>
      Promise.resolve({
        items: [
          { fields: { title: 'Course 1' } },
          { fields: { title: 'Course 2' } },
        ],
      })
    ),
  })),
...
const { getCoursesByCategory, getLesson } = require('../routes/courses');

describe('Courses', () => {
  it('should render list of courses by categories', async () => {
    const result = await getCoursesByCategory('en-US');
...
describe('Lessons', () => {
  it('should render a lesson', async () => {
    const result = await getLesson('en-US', 'example-slug');
    expect(result).toEqual([
      { fields: { title: 'Course 1' } },
      { fields: { title: 'Course 2' } },
    ]);
